...
Create a file named `motd.j2` in the templates directory:

[source,bash,role=copypaste]
----
cat > ~/lab_inventory/templates/motd.j2 << 'EOF'
...
image::exercise-1-4-templates/system_setup_output.png[system_setup.yml output,link=self,window=_blank,width=60%]
...
image::exercise-1-4-templates/motd_verification_output.png[MOTD verification output,link=self,window=_blank,width=60%]
