...
== Create a `requirements.yml` file and run it

A `requirements.yml` file is the standard way to declare collection dependencies in Ansible projects. This makes your automation portable — anyone running your playbook can install all required collections with a single command. Create a `requirements.yml` file under `~/lab_inventory` with the following content:
...
== Collection Execution and Validation
...
image::exercise-1-5-collections/deploy_apache_output.png[deploy_apache.yml output,link=self,window=_blank,width=60%]
...
image::exercise-1-5-collections/curl_output.png[curl output,link=self,window=_blank,width=60%]
