...
Within *Automation Execution → Infrastructure → Inventories*, select the `Workshop Inventory`. Click on the *Hosts* tab and click on `node02`. Click the kebab menu (⋮) and select *Edit host*, then add the `stage: prod` variable in the *Variables* window. This overrides the inventory variable due to order of operations of how the variables are accessed during playbook execution.

image::exercise-2-7-wrap/edit_host.png[edit host,link=self,window=_blank,width=60%]
...
image::exercise-2-7-wrap/web_content_job_template.png[web_content_job_template,link=self,window=_blank,width=60%]
